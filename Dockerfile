...
LABEL description="Mediawiki==1.28.2 with nginx and php5-fpm"

RUN wget -O- https://www.mediawiki.org/keys/keys.txt | gpg --import

RUN wget -O /tmp/mediawiki.tgz https://releases.wikimedia.org/mediawiki/1.28/mediawiki-1.28.2.tar.gz && \
    wget -O /tmp/mediawiki.sig https://releases.wikimedia.org/mediawiki/1.28/mediawiki-1.28.2.tar.gz.sig && \
